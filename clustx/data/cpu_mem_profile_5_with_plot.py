#!/usr/bin/env python3
# Profile CPU & memory usage of multiple clusterers on aggregation.csv
...
def save_cluster_plot(X_raw: np.ndarray, labels: np.ndarray, out_dir: str, method_name: str):
    ...
    """
    labels = np.asarray(labels)
    fname_png = os.path.join(out_dir, f"aggregation_{_slugify(method_name)}.png")
    fname_pdf = os.path.join(out_dir, f"aggregation_{_slugify(method_name)}.pdf")
...
def run_fixed_clustx_from_json(CSV: str, json_path: str, label_col: str = "class") -> Dict:
    """
    Measure CPU & memory for ONE full clustering pass with fixed parameters
    (taken from best_params_summary.json):

      - grid binning
      - dense/sparse/empty mask construction
      - adaptive diffusion weights
      - diffusion
      - seeded O-CCA
      - mapping labels back to points

    Metrics (ARI, NMI, silhouette, etc.) are computed AFTER profiling so
    they do not contaminate time_sec / memory.
    """
    # ----- Load best params from full pipeline -----
    with open(json_path, "r") as f:
        summary = json.load(f)
    ...
    # ----- Load dataset (unprofiled) -----
    X_raw, y_true_local = load_dataset(CSV, label_col)

    # ----- Define the *full clustering pass* to profile -----
    def _clustx_core():
        # bounds with tiny padding (mirror run_pipeline_2d)
        x_min = float(X_raw[:, 0].min()); x_max = float(X_raw[:, 0].max())
        y_min = float(X_raw[:, 1].min()); y_max = float(X_raw[:, 1].max())
        pad_x = 1e-9 * (x_max - x_min + 1.0); pad_y = 1e-9 * (y_max - y_min + 1.0)
        x_min_loc = x_min - pad_x
        x_max_loc = x_max + pad_x
        y_min_loc = y_min - pad_y
        y_max_loc = y_max + pad_y

        # grid + counts
        counts, dx, dy = bin_points_rect(X_raw, nx, ny, x_min_loc, x_max_loc, y_min_loc, y_max_loc)
        norm, _, _ = normalize_nonzero(counts)
        nonempty = (counts > 0)
        ...
        # adaptive diffusion weight threshold (same logic as pipeline)
        if nonempty.any():
            dense_frac = float(dense_before.sum() / max(1, nonempty.sum()))
            q_equiv = max(0.0, min(1.0, 1.0 - dense_frac))
            vals = norm[nonempty]
        ...
        # map back to points
        y_pred = points_to_labels(X_raw, labels_after,
                                  x_min_loc, x_max_loc, y_min_loc, y_max_loc,
                                  dx, dy)
        return y_pred

    # ----- Profile the whole clustering pass -----
    y_pred, prof = run_profiled(_clustx_core)

    # ----- Metrics after profiling (not counted in time/mem) -----
    metrics = score_all(X_raw, y_true_local, y_pred)

    return {"y_pred": y_pred, "metrics": metrics, "profile": prof}


def save_cluster_size_plot(labels: np.ndarray, out_dir: str, method_name: str):
    """
    Save a bar plot of cluster size distribution:
    - labels: cluster labels (N,)
    - out_dir/method_name_cluster_sizes.(png|pdf)

    Noise label (-1) is shown separately (if present).
    """
    labels = np.asarray(labels)
    slug = _slugify(method_name)
    fname_png = os.path.join(out_dir, f"aggregation_{slug}_cluster_sizes.png")
    fname_pdf = os.path.join(out_dir, f"aggregation_{slug}_cluster_sizes.pdf")

    # Separate noise and proper clusters
    noise_mask = (labels < 0)
    has_noise = bool(np.any(noise_mask))

    # Proper clusters (id >= 0)
    cluster_mask = (labels >= 0)
    uniq, counts = np.unique(labels[cluster_mask], return_counts=True)

    # Handle degenerate case: all noise
    if uniq.size == 0:
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.text(0.5, 0.5, "No clusters (all noise)", ha="center", va="center", fontsize=mpl.rcParams['font.size'])
        ax.set_axis_off()
        plt.tight_layout()
        plt.savefig(fname_png, dpi=300, pad_inches=0.05)
        plt.savefig(fname_pdf, dpi=300, pad_inches=0.05)
        plt.close(fig)
        return

    # Sort clusters by size (descending) for nicer visualization
    ...
    ax.set_title(f"{method_name}: cluster size distribution")

    # If many clusters, avoid overcrowded tick labels
    if len(uniq) <= 25:
        ax.set_xticks(x)
        ax.set_xticklabels([str(int(c)) for c in uniq], rotation=45, ha="right")
    else:
        # Sparse ticks
        step = max(1, len(uniq) // 20)
        sel = np.arange(0, len(uniq), step)
        ax.set_xticks(sel)
        ax.set_xticklabels([str(int(uniq[i])) for i in sel], rotation=45, ha="right")

    # Optionally annotate noise size
    if has_noise:
        noise_count = int(np.sum(noise_mask))
        txt = f"Noise (label = -1): {noise_count} points"
        ax.text(0.99, 0.95, txt,
                ha="right", va="top",
...
# ------------------------ Main: aggregation.csv ------------------------

if __name__ == "__main__":
    HERE = os.path.dirname(__file__) if "__file__" in globals() else "."
    CSV = os.path.join(HERE, "aggregation.csv")  # change if needed
    LABEL_COL = "class"

    # Output dirs for metrics + plots
    OUT = os.path.join(HERE, "out_profile_aggregation_with_plot_fixedGridPipeline_with_sizeDist")
    ...
    def _run_de_grid():
        return run_clustx(
            points_file=CSV,
            out_dir=os.path.join(HERE, "out_aggregation_profile"),
            TUNING="bo",
    ...
    JSON_PATH = os.path.join(HERE, "out_aggregation_profile", "best_params_summary.json")

    # ---- DE-Grid+OCCA (single pass with fixed params) ----
    res_stage2 = run_fixed_clustx_from_json(CSV, JSON_PATH, LABEL_COL)
    prof_stage2 = res_stage2["profile"]
    rows.append({"method": "DE-Grid+OCCA (fixed params, full clustering)", **prof_stage2, **res_stage2["metrics"]})

    # Plot using this y_pred
    #if "y_pred" in res_stage2:
    #    save_cluster_plot(X_raw, res_stage2["y_pred"], PLOTDIR, "DE-Grid+OCCA")

    if "y_pred" in res_stage2:
        save_cluster_plot(X_raw, res_stage2["y_pred"], PLOTDIR, "DE-Grid+OCCA")
        save_cluster_size_plot(res_stage2["y_pred"], PLOTDIR, "DE-Grid+OCCA")
    ...
    # ---- Baselines & params (aggregation defaults) ----
    #K = 7
    #EPS, MINPTS = 0.25, 25
    ...
    save_cluster_size_plot(labels, PLOTDIR, "KMeans")
    ...
    save_cluster_size_plot(labels, PLOTDIR, "GMM")
    ...
    save_cluster_size_plot(labels, PLOTDIR, "Agglomerative")
    ...
    save_cluster_size_plot(labels, PLOTDIR, "DBSCAN")
    ...
    if HAVE_HDBSCAN:
        try:
            ...
            save_cluster_size_plot(labels, PLOTDIR, "HDBSCAN")
    ...
    if HAVE_CLIQUE:
        try:
            ...
            save_cluster_size_plot(labels, PLOTDIR, "CLIQUE")
        except Exception as e:
    ...
    print("\n=== aggregation: CPU & Memory Profile (ΔRSS) ===")
    print(preview.to_string(index=False))

    # Save CSV/JSON
    df.to_csv(os.path.join(OUT, "aggregation_cpu_mem.csv"), index=False)
    with open(os.path.join(OUT, "aggregation_cpu_mem.json"), "w") as f:
